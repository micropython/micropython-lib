    ...
    for idx,marg in enumerate(args):
     ...
     if ret.endswith(sep) or ret.endswith(altsep) or \
      ...
      if (ret.endswith(sep) or ret.endswith(altsep)) and \
         (marg.startswith(sep) or marg.startswith(altsep) ):
       if idx != (len(args)-1): doSep = sep
       else: doSep = ''
       ret += (marg.lstrip(_get_bothseps(marg)) + doSep)
      elif idx != (len(args)-1):
       ret += (marg.rstrip(_get_bothseps(marg)) + sep)
      else:
       ret += (marg)
...
def isdir(path):
    ...
    try:
        rstat = uos.stat(path)
        mode = None
        if not rstat: return False #path not exist?
        if isinstance(rstat, (list, tuple)): mode = rstat[0]
        else: mode = rstat.st_mode
        return s_isdir(mode)
    except OSError:
        return False

...
class SystemRandom_TestBasicOps(TestBasicOps, unittest.TestCase):
    ...
    @unittest.skip("pickling is undeveloped")
    def test_pickling(self):
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            with self.assertRaises(NotImplementedError):
                pickle.dumps(self.gen, proto)

    def test_24_bits_per_float(self):
        # This should pass whenever a float has 24 bit precision.
        span = 2 ** 24
        cum = 0
        for i in range(100):
    ...
    def test_rangelimits(self):
        for start, stop in [(-2,0), (-(2**29)-2,-(2**29)), (2**29,2**29+2)]:
    ...
    def test_randbelow_logic(self, _log_=log, int=int):
        ...
        def _log(a, b):
            if b == 2:
                x = 0
                t = a
                while _log_(t, 2) > 1e10: # inf
                    # take big steps first
                    x += 96
                    t >>= 96
                if x:
                    x -= 96
...
class MersenneTwister_TestBasicOps(TestBasicOps, unittest.TestCase):
    ...
    @unittest.skip("specific to Mersenne Twister")
    ...
    def test_setstate_middle_arg(self):
        ...
        self.assertRaises(TypeError, self.gen.setstate, ('uPy:0.1', None, None))
        ...
        self.assertRaises(TypeError, self.gen.setstate, ('uPy:0.1', state, None))
    ...
    def test_long_seed(self):
        ...
        seed = (1 << (100 * 8)) - 1  # about 1K byte
        self.gen.seed(seed)

    def test_bits_per_float(self):
        if maxsize > 1<<40:
            span = 2 ** 53
        else:
            span = 2 ** 24
    ...
    def test_rangelimits(self):
        if maxsize > 1<<40:
            ...
            bits = 29
        for start, stop in [(-2,0), (-(2**bits)-2,-(2**bits)), (2**bits,2**bits+2)]:
            self.assertEqual(set(range(start,stop)),
                set([self.gen.randrange(start,stop) for i in range(100)]))
    ...
    def test_avg_std(self):
        ...
        def xgen(n):
            ...
            for i in range(n-1,0,-1):
                ...
                yield i/n
        '''
        # quick inline test of generator replacement for list:
        xpop = xgen(N)
        xran = lambda: xpop.send(None)
        gran = x[:].pop
        ...
        for variate, args, mu, sigmasqrd in [
            ...
            #g.random = x[:].pop
            xpop = xgen(N)
            g.random = lambda: xpop.send(None)
            y = []
            '''
            # Code was:
            for i in range(len(x)):
...
class TestModule(unittest.TestCase):
    def testMagicConstants(self):
        self.assertAlmostEqual(random.NV_MAGICCONST, 1.71552776992141, places=6)
        self.assertAlmostEqual(random.TWOPI, 6.28318530718, places=6)
        self.assertAlmostEqual(random.LOG4, 1.38629436111989, places=6)
        self.assertAlmostEqual(random.SG_MAGICCONST, 2.50407739677627, places=6)
